FROM ubuntu:18.04
HEALTHCHECK --start-period=5m CMD /healthcheck-4.1.x.sh
...
RUN \
  ...
  apt-get update &&\
  DEBIAN_FRONTEND="noninteractive" apt-get install --no-install-recommends -y gosu mongodb-server-core net-tools openjdk-8-jre-headless tzdata wget authbind &&\
...
ENTRYPOINT ["/entrypoint-4.1.x.sh"]
